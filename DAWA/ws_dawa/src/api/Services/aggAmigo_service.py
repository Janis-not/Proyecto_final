...
from ws_dawa.src.api.Components.aggAmigo_component import AggAmigoComponent
from ws_dawa.src.api.Model.Request.agg_request import AggRequest
from flask import request
from flask_restful import Resource


class AggService(Resource):
    @staticmethod
    def post():
        try:
            ...
            new_request = AggRequest()
            error_en_validacion = new_request.validate(rq_json)
            if error_en_validacion:
                HandleLogs.write_error("Error al validar el request -> " + str(error_en_validacion))
                return response_error("Error al validar el request -> " + str(error_en_validacion))

            resultado = AggAmigoComponent.aggAmigo(rq_json['estudiante_id'], rq_json['seguido_id'])
            if resultado['result']:
