...
from ws_dawa.src.api.Components.carrera_component import CarreraComponent
from flask import request
from flask_restful import Resource


class CarreraService(Resource):
    @staticmethod
    def get():
        try:
            HandleLogs.write_log("Ejecutando servicio de Listar Usuario")
            c_facultad_id = request.args.get('id')

            if not c_facultad_id:
                return response_error("El parámetro 'id' es requerido")

            resultado = CarreraComponent.getAllcarrera(c_facultad_id)
            if resultado['result']:
